...
public class StatSequence<E_STAT_TYPES, E_STAT_INTERACTION_TYPES> : Stat<E_STAT_TYPES, E_STAT_INTERACTION_TYPES>
    where E_STAT_TYPES : Enum
    where E_STAT_INTERACTION_TYPES : Enum {

    private IntFilterSequence stat = new IntFilterSequence();
    ...
    public StatSequence() : base(Classes.SEQUENCE) {

        this.asStatSequence = this;
    }
    ...
    public int GetStatFiltered(int valueToFilter) {
        return stat.ApplyFilters(valueToFilter);
    }
