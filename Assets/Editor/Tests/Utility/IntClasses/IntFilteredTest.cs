...
namespace Tests {
    ...
    /// </summary>
    [TestFixture]
    public class IntFilteredTest {

        [Test]
        public void Base_Int_Changeable() {
            const int BASE_INT = 20;
            const int NEW_BASE_INT = 30;

            // Create new filter int
            IntFiltered fint = new IntFiltered(BASE_INT);
            int baseInt = fint.GetBaseInt();
            Assert.AreEqual(BASE_INT, baseInt);

            // Change the filter Base Int and Test
            fint.SetBaseInt(NEW_BASE_INT);
            baseInt = fint.GetBaseInt();
            Assert.AreEqual(NEW_BASE_INT, baseInt);
